export module initd3d12x:approach2.d3d12xapp;
import std;
import shared;
import :common;

// Approach 2: This is more concise, but less reusable and testable than the first approach. 
// The D3D12 state is now directly embedded in the application class, and the application 
// class is responsible for initializing it. This is a more traditional approach, but it 
// can make testing and reusability more difficult, as the D3D12 state is tightly coupled to 
// the application class.
export namespace Approach2
{
	class D3D12xApp
	{
	public:
	...
	private:
		void InitialiseD3D12(this D3D12xApp& self)
		{
		...
		auto CreateSwapChain(this auto& self) -> decltype(auto)
		{
			...
			auto swapChainDesc = DXGI::DXGI_SWAP_CHAIN_DESC{
				...
				.OutputWindow = self.window.GetHandle(),
				.Windowed = true,
		...
		auto CreateDepthStencilView(this auto& self) -> decltype(auto)
		{
		...
		auto SetViewportAndScissorRect(this auto& self) -> decltype(auto)
		{
			auto viewport = D3D12::D3D12_VIEWPORT{
				.TopLeftX = 0.0f,
				.TopLeftY = 0.0f,
				.Width = static_cast<float>(self.window.GetWidth()),
				.Height = static_cast<float>(self.window.GetHeight()),
				.MinDepth = 0.0f,
				.MaxDepth = 1.0f
			};
			auto scissorRect = D3D12::D3D12_RECT{
				.left = 0,
				.top = 0,
				.right = static_cast<Win32::LONG>(self.window.GetWidth()),
				.bottom = static_cast<Win32::LONG>(self.window.GetHeight())
			};
			return self;
		}
	};
